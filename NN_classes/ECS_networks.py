...
class NN(NN_class):
    '''
    Methods such as train/predict are derived from base NN_CLass
    '''
    def __init__(self, t, X, Y, layers,
                 train_on_last=True,
                 activation = tf.keras.activations.sigmoid):
        NN_class.__init__(self, t, X, Y, layers,train_on_last,activation)
        self.Model_name = 'NN'
...
class PyNN(NN_class):
    '''
    Physics based Neural Network for RC circuit.
    Methods such as train/predict are derived from base NN_CLass
    Network and Loss function are overwritten
    '''
    def __init__(self, t, X, Y, layers,
                 train_on_last=True,
                 activation = tf.keras.activations.sigmoid):
        NN_class.__init__(self, t, X, Y, layers,train_on_last,activation)
        self.Model_name = 'PyNN'
    ...
    def Network(self,X,t,Y_prev=None):
        ...
        :returns :f = dy/dt + (y-V)/RC
        Note: Important assumption that we assume Q_process to be constant = average value of all Q_processes used in
        simulation
        '''
        ...
        self.NonlinOP = Calculate_NonLinearOP(Fan_powers=X,
                                              Weather_data_tf=self.Weather_data_tf,
                                              T_basin=y_unnorm)
...
class PyLSTM(NN_class):
    ...
    def LSTM(self, X,t,Y_prev, weights, biases):
        # num_layers = len(weights) + 1
        # X.shape = batch_size, chunk,size
        X = (X - self.Xmean) / self.Xsd
        Y_prev = (Y_prev - self.Ymean) / self.Ysd
        H = tf.concat((X,t,Y_prev),axis=1)
    ...
    def Network(self, X,t,Y_prev=None):
        ...
        Y_prev = tf.concat((first_val, Y_prev),axis=0)
        y_out = self.LSTM(X,t,Y_prev, self.weights, self.biases)
        y_unnorm = (y_out * self.Ysd + self.Ymean)[:, 0:1]
